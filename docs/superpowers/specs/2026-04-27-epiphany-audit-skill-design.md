...
- `--full-rerun` / `--no-rerun` — override the §2.5 tiered audit-rerun policy. `--full-rerun` forces full N01..N14 Pass A rerun (high-stakes runs); `--no-rerun` skips the audit-rerun sub-step entirely (token-budget-constrained runs; fix-report records `audit_rerun_delta: skipped-by-flag`). Mutually exclusive; passing both → `halt-on-flag-conflict`. Both valid only with `--fix` or no-flag (fix-mode); ignored with warning under `--audit`. Under `--dry-run`, `--no-rerun` is moot (the dry-run plan terminates at N17 before any audit-rerun could occur); emit warning *"--no-rerun has no effect under --dry-run; the dry-run plan terminates before the audit-rerun step."*
- `--reverify-state` — at `--fix` mode entry, clear all `reachable: false` annotations from the state file at `~/docs/epiphany/audit/.state/<source-report-id>.json`. The run then re-checks each affected finding against git history per the standard idempotency-check flow (per §6.1 conflict resolution). Use case: the user previously chose `(b) skip [trust state]` for a state-file-vs-git-log conflict and now wants to re-evaluate (e.g., after a branch restore or history un-rewrite). Valid in `--fix` mode only; ignored with warning otherwise.
...
| N16 | FixTriage (F-VAL ingest + Triage) | validator + triage | inline | **Folded from input-prompt N17+N18.** **Resume-handler sub-step (when entered via `resume` from `halt-on-recovery-conflict`):** runs as the FIRST sub-step of N16's ingest, before F-VAL. Performs the tree-divergence safety check (per §6.1), the broad pre-flight cleanup (`git checkout -- . && git clean -fd` — only after safety check passes or user authorizes), and updates the recovery manifest (moves `in_flight_finding_id` back to `pending`). Halt conditions: `halt-on-resume-tree-divergence` (b-choice), `halt-on-user-abort` (ctrl-C). Skipped on fresh `--fix` runs. Then proceeds with the standard ingest flow. Schema-validates input report (`schemas/audit-report-v1.schema.json`); halt with precise field/finding-id error on schema fail; SHA-256 of source audit report captured for `source_audit_report_sha256`. **Empty-or-unfixable check (early halt):** if source audit report has zero main-body findings (excluding "Unverified Hypotheses"), halt with `halt-on-empty-or-unfixable-report` *before* running idempotency / triage / suspicious-content checks. **Idempotency check at ingest time** (state file authoritative; git-log fallback). State-file entries with `reachable: false` annotations are skipped silently (no re-prompt) unless `--reverify-state` was passed (which clears all such annotations at run entry). Suspicious-content prompt overrides `--auto`. Groups by file/module; topo-sorts; tier 1/2/3 classification (rules per §2.3); defer-on-uncertainty; conflicting-edit detection: in live mode → `halt-on-conflicting-fixes`; under `--dry-run` → record in `triage_summary.conflicting_groups` and continue (the plan serves as the conflict report; no halt). **Owns fix-side CONDITIONAL ROUTING.** | aggregator (file-grouping) |
...
- **Floor-plugin missing detection:** if a built-in floor plugin file (`correctness.md` or `maintainability.md`) is **missing** from `~/.claude/skills/epiphany-audit/dimensions/`, halt with `halt-on-floor-plugin-missing` (subreason: `<plugin-name>`). The bundle is corrupt and proceeding would produce a defective audit. The floor cannot be silently degraded by hardcoded fallbacks. (This is distinct from schema-validation failure on a present-but-malformed file, which is logged and skipped — but a floor plugin failing schema validation also halts via this rule.)
...
| E13 | N16 → N17 → N18 → N19 → N20 → N21 → N23 | data/control chain | 1:1 each | fix pipeline; entry from `--fix` mode or post-E21 fix-offer consent. (Success-path flow continues from N23 via E_finalize → N22 → E_complete → user; halt-mid-fix-\* paths route from N19/N20/N21 via E_halt_partial → N23 → user without reaching N22.) |
...
| `halt-on-resume-tree-divergence` | resume-handler (§6.1) | resume's pre-flight cleanup would discard working-tree changes outside the prior run's scope (files not in the in-flight finding's `location` AND not in the audit's flagged-files set). User choice = (a) discard and resume, (b) abort and reconcile manually. |
| `halt-on-floor-plugin-missing` | N02 R-ROUTE startup | a built-in floor plugin file (`correctness.md` or `maintainability.md`) is missing from `~/.claude/skills/epiphany-audit/dimensions/`. Subreason: `<plugin-name>`. The bundle is corrupt; floor cannot be silently degraded. |
...
provenance:                          # I2 — finding-level trace; mandatory on every finding
  ...
  model: <model-id>                  # e.g., "claude-opus-4-7". Population:
                                     #   - inline-produced findings: orchestrator's model.
                                     #   - subagent-produced findings (analyzer fan-out under --deep,
                                     #     narrow-rerun analyzers): the SUBAGENT's model.
                                     #   When a finding is initially produced inline and later demoted
                                     #   by a subagent (Pass B), `model` stays the orchestrator's model
                                     #   and `pass_b_model` is populated with the Pass B subagent's model.
  pass_b_model: null | <model-id>    # populated when q_gate_pass_b_demoted: true; the Pass B subagent's
                                     # model. Allows determinism harness to detect Pass B model upgrades
                                     # as infrastructure drift rather than content drift.
...
| `intentional.value == true` | **Demote to Unverified Hypotheses** with `notes: "demoted: code appears intentional ({{intentional.justification}})"`. The justification from `false_positive_check.intentional` is interpolated. |
| `reachable_from_entry.value == false` AND `severity ∈ {CRITICAL, HIGH}` | **Demote to Unverified Hypotheses** with `notes: "demoted: unreachable from declared entry points but claimed {{severity}} severity — likely overestimated impact"`. |
...
| `fix_breaks_dependents.value == true` AND `confidence == HIGH` | **Pass**; the remediation field MUST address the dependents (e.g., paired updates). |
| `fix_breaks_dependents.value == true` AND `confidence < HIGH` | **Demote to Unverified Hypotheses** with `notes: "demoted: fix would break dependents ({{fix_breaks_dependents.justification}}); confidence below HIGH floor for breaking-change findings"`. |
...
#   At most one of {auto, confirm-all}.
#   verbose, deep are orthogonal.
#   `dry-run` does NOT appear in fix-report flags — under --dry-run the pipeline halts at N17
#   and emits a Dry-Run Plan v1 (§4.4) instead of a fix report. The dry-run plan's flags
#   field carries the dry-run flag.
#   Allowed sets:
#     []  [auto]  [confirm-all]
#     [verbose]  [deep]  [verbose, deep]
...
post_metrics:
  ...
  build:      pass | fail
                  # post_metrics: null on partial reports (halt-mid-fix-* before N21 battery completed)

audit_rerun_delta:
  scope: full | narrow | skipped-tier-policy | skipped-by-flag | null
                  # null when N21 did not execute (run halted before regression battery — e.g., halt-mid-fix-on-perfix-cap-hit fired during atomic-loop)
                  ...
                  # full: Tier-3 present (or --full-rerun), audit-rerun ran N01..N14 Pass A (Pass B skipped)
...
diff_scope_check: pass | fail-with-unmapped-hunks | n/a   # n/a on partial reports where N21 didn't run
...
- **Partial-report invariant (fix-report):** `partial: true ⇔ halt_state: non-null` (mutual implication; validated by JSON-Schema). When `partial: true`, the body MAY contain fewer entries than the source audit report's findings — entries reflect work-to-date up to the halt point (verified, failed, deferred so far); when `partial: false`, the body has exactly one entry per source-audit finding (or its derived induced-regression / new-finding descendants). On partial reports, `audit_rerun_delta.scope` MAY be `null` (run halted before regression battery), `post_metrics` MAY be `null`, `diff_scope_check` MAY be `n/a`, and `last_known_good_sha` is populated iff ≥1 fix-group successfully committed before the halt.
...
| 8. Audit-rerun (tiered, per §2.5) | N21 (audit-rerun sub-step) | Tier-1-only run → skip; Tier-2-only run → narrow rerun (fix-dimension analyzers + minimal post-pipeline subset: N10 FPV, N11, N12, N14 Pass A); Tier-3 present → full N01..N14 Pass A rerun (Pass B skipped). Classify new findings (touched vs. untouched files). Runs **after** regression battery — only proceeds if battery passes. |
...
**Paired follow-up commit format** (used when N19's policy mandates a separate commit per §5.2 step 9 — e.g., language tooling rejects bundled test+source commits, or a project-level pre-commit hook rejects the bundled commit): the follow-up uses the tag `[AUDIT-NNN-test]`, where `NNN` matches the primary commit's tag. Examples: `F001` primary commit is `[AUDIT-001]`; its paired test commit is `[AUDIT-001-test]`. The diff-scope check in N21 (per §5.1, §2 N21) treats `[AUDIT-NNN]` and `[AUDIT-NNN-test]` as **paired** — both diffs count as in-scope for the same finding. Body template for the test commit:

```
[AUDIT-NNN-test] regression test for <one-line summary>

Test-for-finding: F00N
Dimensions: <comma-separated, mirrors the primary commit>
Severity: <mirrors the primary commit>
Source: <basename of source audit report>
```

The fix report records the test commit's sha in `regression_test_added.deferred_to_followup_commit`.
...
- Input report references files outside `audit_target` git tree → `halt-on-files-outside-tree`; refuse to write outside source tree.
...
- Recovery manifest detected at run start → `halt-on-recovery-conflict`; user choice:
    - `resume` — pick up from `last_known_good_sha`; skip applied; continue with pending list. **Tree-divergence safety check (mandatory, before any cleanup):** the resume-handler runs `git status --porcelain` and `git diff --name-only HEAD`. If the divergence includes any file outside the in-flight finding's `location` file AND outside the audit's flagged-files set, halt with `halt-on-resume-tree-divergence` and prompt: *"Working tree contains changes outside the prior run's scope: [list]. These will be discarded by `git checkout -- . && git clean -fd` if you continue. Options: (a) discard and resume, (b) abort and reconcile manually."* Only run the broad cleanup after explicit user authorization (a). On (b), halt with no working-tree changes. **In-flight-fix-group restart semantics (per O3):** if the manifest shows `in_flight_finding_id` set without a corresponding `applied` or `failed` entry (i.e., process died inside the atomic loop between fix-group boundaries), the resume-handler runs the pre-flight cleanup (`git checkout -- . && git clean -fd` — only after the safety check above passed or the user authorized) and **restarts that fix-group from scratch** as fix-group iteration #1. The `in_flight_finding_id` is moved back to `pending` in the manifest. No half-applied state is possible because the atomic loop never commits before verification passes.
