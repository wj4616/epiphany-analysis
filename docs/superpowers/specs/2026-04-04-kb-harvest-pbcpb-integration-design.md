...
"source": {
  ...
  "reference": "...",
  "url": "...",
  "backend": "research-docs | firecrawl | ddg+webfetch | websearch+webfetch | webfetch",
  "retrieved_date": "2026-04-04"
}
```

For existing entries that lack `source.backend`, validators and skills infer `"research-docs"` as default. This is a non-breaking addition — old entries remain valid.
...
"harvest_metadata": {
  ...
  "field_provenance": {
    ...
    "concepts": { "method": "ai-extracted", "confidence": 0.80 },
...
`overall_confidence = sum(field_confidence * weight * source_domain_weight)`
  ...
  "topics": [
    {
      ...
      "files": {
        "<filename>.json": {
          ...
          "has_semantic": true,
          "source": "curated | ddg+webfetch | websearch+webfetch | webfetch | firecrawl | research-docs",
  ...
  "registries": [
    {
      ...
      "schema_path": "/home/myuser/playbooks/vst-product-lifecycle-playbook/architecture/entry-schema.json",
...
- harvest_metadata.source_content_length: word count of source content
...
harvest-status.json: { "layer": { "total": N, "harvested": N, "placeholder": N } }
```
...
BATCH COMPLETE (N entries written)
    ...
    |     ├── For entry-level cross_references[]:
    |     |   ├── Only when concept name exactly matches concept in another KB's entry
    |     |   ├── Lookup via master-index (O(1) per concept)
    |     |   └── Add cross_reference in BOTH entries (bidirectional)
    |     |         Note: modifying entries in other KBs is metadata-only, no version bump
    |     |         Only add if target entry's schema supports cross_references field
    ...
    |     ├── Mark used search terms in search-terms.json
    |     ├── Add any new terms discovered from harvested content
...
Resume: `kb-harvest --kb <name> --resume` reads checkpoint and continues from `in_progress` / `pending` entries. Skips `completed` and `fetch_failed`.
...
- [ ] Cross-references maintained bidirectionally during cascade
