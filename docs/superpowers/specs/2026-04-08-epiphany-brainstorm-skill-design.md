# Epiphany Brainstorm Skill — Design Spec

- **Date:** 2026-04-08
- **Status:** Validated design, ready for implementation plan
- **Target file:** `~/.claude/skills/epiphany-brainstorm/SKILL.md`
- **Neighbors for style reference:** `~/.claude/skills/prompt-epiphany/SKILL.md`, `~/.claude/skills/brainstorming-epiphany/SKILL.md`
- **Source brief:** `~/prompts/brainstorm/epiphany-brainstorm-skill-design-verbose.md`
- **Standalone requirement:** Yes. No runtime reads of any sibling skill, spec, or external file. Integration with `prompt-epiphany` or a future "ultimate" combined skill is **manual and user-directed**; no runtime calls.

## Design Decisions Log

| # | Decision | Rationale |
|---|---|---|
| D1 | Methodology curation: SCAMPER + Lateral Thinking / Morphological Analysis / Six Thinking Hats / TRIZ / Reverse Brainstorming / Pugh Matrix | Non-redundant coverage of the six required categories (divergent ideation / systematic completeness / multi-perspective critique / contradiction resolution / risk capture / decision selection); every included item is a distinct KB methodology |
| D2 | Lens execution style: "Layered Lenses" — each lens can see prior lens outputs but is protected by explicit firewalls | User chose this over strict fresh-read segregation because layered visibility yields better results when firewalls are enforced |
| D3 | Three firewalls (merged from an initial four) | "Fresh-read" and "generate first, compare second" had overlap; merged into a single ordered rule. Net three rules with the same coverage |
| D4 | Six Thinking Hats content fully inlined in the SKILL.md, not referenced at runtime | User explicit instruction: skill must operate standalone; implementation of the separate six-thinking-hats skill is deferred |
| D5 | MH-10's runtime Six Hats spec reference is **superseded** by the standalone requirement | Six Hats content was drawn from the `2026-04-07-six-thinking-hats-skill-design.md` spec at **design time**; no runtime file dependency exists |
| D6 | Synthesis checkpoint runs **before** the decision stage | Resolves an inconsistency between the brief's examples 2 and 3; synthesis feeds Pugh Matrix inputs, so it must precede decision |
| D7 | Scale router accepts `--minimal` / `--standard` / `--deep` flag overrides at first or last token | Mirrors the `prompt-epiphany` flag convention for consistency; `--standard` is accepted but never required |
| D8 | S5 = Reverse Brainstorming (distinct KB methodology), not a "Black Hat extended + TRIZ physical" hybrid | Earlier hybrid design blurred S3/S4/S5 boundaries and duplicated TRIZ work; Reverse Brainstorming is distinct and justifies being DEEP-only |
| D9 | Design spec saved to `docs/superpowers/specs/`; runtime output saved to `~/prompts/brainstorm/` | Matches local conventions and the brief's `<defaults>` |
| D10 | MINIMAL Six Hats is reduced to Black + Yellow + Green only | At MINIMAL scale the input hasn't earned process management (Blue), neutral data gathering (White), or emotional assessment (Red); the three retained hats cover risk / benefit / alternatives which is the minimum viable critique |
| D11 | Synthesis checkpoint and all validation gates do **not** count toward methodology budget | Only methodology lenses (S1–S6) count; this keeps budget arithmetic clean |
| D12 | `<brainstorm_output_v1>` is a **stable manual-merge anchor**, not a runtime hook | The future "ultimate" combined skill will read this block and merge it with a genius-mind counterpart by hand; this skill never calls anything |
...
- **Methodology source:** KB §1.3 Six Thinking Hats (Edward de Bono, 1985). Content derived at design time from the `2026-04-07-six-thinking-hats-skill-design.md` spec; inlined here for standalone runtime operation.
- **AI-agent adaptation:** In a human team, "everyone wears the same hat simultaneously" — a facilitator manages hat transitions and keeps the group in parallel mode. A single AI agent cannot literally wear hats simultaneously, so the adaptation is sequential: the agent adopts each hat's mode one at a time, fully exits the hat before the next, and the firewalls replace the social ritual of facilitation.
...
## 11. Output Format (runtime deliverable)
...
## 16. Verification Checklist (for design self-review and implementation plan)
...
| V10 | Save behavior present — explicit offer with `~/prompts/brainstorm/<slug>.md` and collision handling | §14 |
| V11 | Six Thinking Hats inlined, not referenced at runtime — no runtime file dependency | §8 S3, §12 clean-separation guarantee |
| V12 | No fabrication — every methodology cited exists in the KB; no invented methodologies or numbers | §9 |
| V13 | Edge case coverage — every edge case from the brief has a corresponding row in the edge cases table | §13 |
| V14 | Quality Standard defines "better than uninstructed LLM output" | §15 |

---

## Appendix A — Relationship to the Source Brief

The source brief (`~/prompts/brainstorm/epiphany-brainstorm-skill-design-verbose.md`) specified MH-1 through MH-10, SH-1 through SH-3, NR-1 through NR-3, a DO/DO-NOT list, defaults, and an output format template. This design honors all of them with **one explicit override**:

- **MH-10 overridden.** The brief required the skill to "reference and follow the Six Thinking Hats skill design spec at the resolved path". The user subsequently instructed the skill must operate **standalone** with no runtime file reads, because the `six-thinking-hats` skill itself has not been implemented (only its design spec exists). Resolution: Six Hats content was drawn from the `2026-04-07-six-thinking-hats-skill-design.md` spec at **design time** and fully **inlined** in §8 S3. The runtime skill reads no external file for Six Hats content.

All other brief requirements are honored without modification.

## Appendix B — Design Session Provenance

This spec was produced through an interactive brainstorming session on 2026-04-08 following the `brainstorming` skill's checklist:

1. Explored project context (prompt-epiphany style, six-thinking-hats spec, existing skills directory)
2. Skipped visual companion (text-only design)
3. Clarifying questions answered interactively
4. Three approaches proposed (Fresh-Read / Layered / Parallel-Simulated); user selected Layered with firewalls
5. Four design sections presented with approval gates (firewalls / pipeline / output / edge-cases)
