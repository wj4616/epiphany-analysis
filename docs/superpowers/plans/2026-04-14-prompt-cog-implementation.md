# prompt-cog Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `~/.claude/skills/prompt-cog/SKILL.md` — a single-file prompt enhancement skill that runs inline analysis/ideation with one synthesis agent spawn, positioned between prompt-epiphany (fast/loose) and epiphany-prompt (modular/slow).

**Architecture:** 1-spawn inline orchestration: Steps 0–2 (flag/routing/announce) → Steps 3–4 (analyst + ideation roles, inline, with structural channel markers) → Step 5 (pre-spawn checkpoint + spawn assembly) → Step 6 (1 synthesis agent spawn) → Step 7 (output + save). All analysis and ideation run in the orchestrator context under role-switched framing. Channel markers (`=== ANALYST/IDEATION OUTPUT BEGIN/END ===`) allow Step 5 to extract content by structural address rather than by parsing prose.

**Tech Stack:** Claude Code skill system (SKILL.md YAML frontmatter + markdown), prompt-epiphany T1–T13 technique definitions (read from source skill), epiphany-prompt KB snippets (3 inlined: chain-of-thought.md, structured-output.md, self-refine.md). No external files created beyond SKILL.md itself.
...
|------|--------|---------------|
| `~/.claude/skills/prompt-cog/SKILL.md` | **Create** | Complete skill definition — all 7 steps, 2 modes, flag handling, synthesis spawn prompt, KB snippets inlined |
| No other files | — | Architecture deliberately eliminates session dir, stage files, module files |

---

### Task 1: Create skill directory and scaffold SKILL.md with frontmatter + trigger conditions

**Files:**
- Create: `~/.claude/skills/prompt-cog/SKILL.md`

The goal of this task is to lay the file's skeleton so every subsequent task can append to a known structure.

- [ ] **Step 1: Verify the skills directory exists**

Run: `ls ~/.claude/skills/ | head -20`
Expected: shows existing skill directories (prompt-epiphany, epiphany-prompt, etc.)

- [ ] **Step 2: Create the skill file with frontmatter and trigger table**

Create `~/.claude/skills/prompt-cog/SKILL.md` with this exact content:
...
```bash
cd /home/myuser
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): scaffold SKILL.md with frontmatter, triggers, and hard gates"
```

---

### Task 2: Step 0 — Flag Detection

**Files:**
- Modify: `~/.claude/skills/prompt-cog/SKILL.md` (append to end)

- [ ] **Step 1: Append Step 0 section**

Append the following to `~/.claude/skills/prompt-cog/SKILL.md`:
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 0 flag detection with E13 disambiguation"
```

---

### Task 3: Step 1 — Input Routing

**Files:**
- Modify: `~/.claude/skills/prompt-cog/SKILL.md` (append)

- [ ] **Step 1: Append Step 1 section**

Append to `~/.claude/skills/prompt-cog/SKILL.md`:
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 1 input routing with Type A/B/C and Type D advisory"
```

---

### Task 4: Step 2 — Announce + Complexity Advisory + Sufficiency

**Files:**
- Modify: `~/.claude/skills/prompt-cog/SKILL.md` (append)

- [ ] **Step 1: Append Step 2 section**
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 2 announce, complexity advisory (E04), sufficiency check"
```

---

### Task 5: Step 3 — Analysis (Analyst Role)
...
**T1–T13 reference:** The 13 technique definitions are in `prompt-epiphany`'s SKILL.md `## Techniques` section at `~/.claude/skills/prompt-epiphany/SKILL.md`. Read that file if the definitions are not already in context.
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 3 analyst role with INVENTORY YAML schema and E14 self-check"
```

---

### Task 6: Step 4 — Ideation (Ideation Role)
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 4 ideation role with anti-conformity, novelty gate (E03), E07 conflict finalization"
```

---

### Task 7: Step 5 — Pre-Spawn Checkpoint
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 5 pre-spawn checkpoint with E01 channel validation and E11 coherence check"
```

---

### Task 8: Step 6 — Synthesis Agent Spawn Prompt

**Files:**
- Modify: `~/.claude/skills/prompt-cog/SKILL.md` (append)

This is the largest task — the synthesis spawn prompt must include the full role declaration, protocol, inlined KB snippets, and return value contract.

- [ ] **Step 1: Append Step 6 section with complete synthesis spawn prompt**
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 6 synthesis agent spawn prompt with inlined KB snippets, E02 placement map, E05 verbatim contract"
```

---

### Task 9: Step 7 — Output and Save
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): Step 7 output and save with E09 FAIL recovery guidance"
```

---

### Task 10: Techniques Section (T1–T13 reference)

**Files:**
- Modify: `~/.claude/skills/prompt-cog/SKILL.md` (append)

Step 3 references T1–T13 from prompt-epiphany's SKILL.md. prompt-cog should include its own copy of the technique definitions so the analyst role can operate without a file read (the read is a fallback). Read the Techniques section from `~/.claude/skills/prompt-epiphany/SKILL.md` and embed it.

- [ ] **Step 1: Append the Techniques section to prompt-cog SKILL.md**
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): embed T1-T13 technique definitions for inline analyst reference"
```

---

### Task 11: Register skill and write smoke test

**Files:**
- Modify: `~/.claude/skills/prompt-cog/SKILL.md` (append quality notes section)
- No separate test file — smoke test is a manual checklist in the SKILL.md

- [ ] **Step 1: Append Design Notes and Smoke Test checklist**
...
```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): design notes and smoke test checklist"
```

---

### Task 12: Final structure verification

**Files:**
- Read: `~/.claude/skills/prompt-cog/SKILL.md`

- [ ] **Step 1: Read the complete SKILL.md and verify all sections present**

Read `~/.claude/skills/prompt-cog/SKILL.md` and confirm the following sections exist in order:
- [ ] YAML frontmatter (name: prompt-cog, version, description)
- [ ] Trigger Conditions table
- [ ] Hard Gates (3 gates)
- [ ] Pipeline diagram
- [ ] Step 0 — Flag Detection
- [ ] Step 1 — Input Routing (Type A/B/C + Type D)
- [ ] Step 2 — Announce + Complexity Advisory + Sufficiency
- [ ] Step 3 — Analysis (ANALYST OUTPUT markers, all mode variants, E14 self-check)
- [ ] Step 4 — Ideation (IDEATION OUTPUT markers, anti-conformity with E03 novelty gate, E07 conflict finalization)
- [ ] Step 5 — Pre-Spawn Checkpoint (6-item checklist, channel boundary check E01, Interface 2 E11, spawn assembly)
- [ ] Step 6 — Synthesis Agent (full spawn prompt with role, Hard Gate 3, E05 verbatim contract, KB snippets ×3, synthesis protocol, placement map E02, return format)
- [ ] Step 7 — Output (PASS/FAIL parse, save path, E09 recovery)
- [ ] Techniques T1–T13
- [ ] Design Notes
- [ ] Smoke Test Checklist
...
Grep for: `TBD`, `TODO`, `fill in`, `implement later`, `similar to Task`, `[Paste`, `[...]` (as unresolved placeholder, not as literal E13 example text).

Run: `grep -n "TBD\|TODO\|fill in\|implement later\|\[Paste" ~/.claude/skills/prompt-cog/SKILL.md`
Expected: No output (zero matches).

- [ ] **Step 3: Verify channel markers spelled consistently**

Run: `grep -n "ANALYST OUTPUT\|IDEATION OUTPUT" ~/.claude/skills/prompt-cog/SKILL.md`
Expected: All four marker variants appear: `=== ANALYST OUTPUT BEGIN ===`, `=== ANALYST OUTPUT END ===`, `=== IDEATION OUTPUT BEGIN ===`, `=== IDEATION OUTPUT END ===`

- [ ] **Step 4: Final commit**

```bash
git add .claude/skills/prompt-cog/SKILL.md
git commit -m "feat(prompt-cog): complete SKILL.md — 1-spawn inline orchestration skill v1.0.0"
```
...
|---|---|
| Overview: 1-spawn architecture | Architecture header, Task 1 frontmatter |
| Flags table (--minimal, --quiet, deferred flags) | Task 1 trigger table, Task 2 Step 0 |
| Flag disambiguation E13 | Task 2 Step 0 |
| Enhancement Inventory verdicts (PORT AS-IS / SIMPLIFIED / DEFER / SKIP) | Reflected in what IS and IS NOT implemented in each step |
| Architecture diagram | Task 2 pipeline diagram |
| Step 0 — Flag Detection | Task 2 |
| Step 1 — Input Routing (A/B/C + Type D E06) | Task 3 |
| Step 2 — Announce + E04 complexity advisory + E12 minimal transparency + Sufficiency | Task 4 |
| Step 3 — 6-dimension analysis, INVENTORY YAML, E14 self-check, channel markers E01 | Task 5 |
| Step 4 — Ideation, anti-conformity + E03 novelty gate, E07 same-target conflict, E10 role transition, channel markers E01 | Task 6 |
| Step 5 — 6-item checklist, E01 boundary check, E11 Interface 2 coherence, spawn assembly | Task 7 |
| Step 6 — Synthesis agent, E05 verbatim contract, E02 placement map, 3 KB snippets, Hard Gate 3, return format | Task 8 |
| Step 7 — PASS/FAIL parse, save path, tilde expansion, collision handling, E09 FAIL recovery | Task 9 |
| T1–T13 reference in analyst context | Task 10 |
| Design decisions DD#1–DD#14 | Task 11 design notes |
| Smoke tests | Task 11 smoke test checklist |
| Out of scope items (repair loops, DEEP, session dir, --verbose/spec/plan) | Reflected in deferred flag handling + design notes |
