# epiphany-genius Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create the epiphany-genius skill file that implements a 4-phase reasoning amplifier pipeline with stakes-based calibration, incubation loops, and verification gates.

**Architecture:** Single SKILL.md file containing frontmatter + 11 sections (Trigger Conditions, Composition Model, Hard Gate, Stakes Assessment, Pipeline with 4 phases, INCUBATE LOOP, Confidence Tier Handling, Output Gate, Output Schema, Corporate Scenario Triggers, Phase ↔ Corporate Vocabulary Mapping). The skill is instruction-only (no code) - it guides an LLM through the reasoning pipeline.

**Tech Stack:** Markdown skill definition (SKILL.md). No external dependencies.

**Spec:** `docs/superpowers/specs/2026-04-08-epiphany-genius-design.md`
...
| File | Purpose |
|------|---------|
| `~/.claude/skills/epiphany-genius/SKILL.md` | Main skill definition - complete pipeline instructions |

---

### Task 1: Create skill directory and initial structure

**Files:**
- Create: `~/.claude/skills/epiphany-genius/SKILL.md`

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p ~/.claude/skills/epiphany-genius
```

- [ ] **Step 2: Write the complete SKILL.md file**

Create `~/.claude/skills/epiphany-genius/SKILL.md` with the following content:
...
```bash
wc -l ~/.claude/skills/epiphany-genius/SKILL.md && head -20 ~/.claude/skills/epiphany-genius/SKILL.md
```

Expected: File shows version 1.0.0, correct name and description in frontmatter, begins with "# Epiphany Genius" heading.

- [ ] **Step 4: Commit**

```bash
cd ~ && git add .claude/skills/epiphany-genius/SKILL.md
git commit -m "feat(epiphany-genius): add skill definition for reasoning amplifier pipeline

Implements 4-phase pipeline (Frame → Diverge → Stress-Test → Synthesize)
with stakes-based calibration, incubation loops, and verification gates.
Based on design spec 2026-04-08-epiphany-genius-design.md."
```
...
- [ ] **Spec coverage:** All content from Sections 1-6 of the design spec is incorporated into SKILL.md
- [ ] **No placeholders:** No TBD, TODO, or "fill in" content
- [ ] **Type consistency:** All referenced parameters (N_framings, loop_budget, bayesian_bar, gap_scan_depth) are defined consistently with spec Section 5.1
- [ ] **Pipeline phases:** All 4 phases have complete instructions matching spec Section 4
- [ ] **Loop table:** All 5 backward edges documented matching spec Section 2
- [ ] **Output gate:** All 7 checks (G1-G7) documented matching spec Section 5.3
- [ ] **XML schema:** Complete schema with required/optional elements marked matching spec Section 5.4
- [ ] **Confidence tiers:** All 6 findings from spec Section 5.2 table documented
- [ ] **Self-contained:** No ANCHOR comments or references to planned second skill (spec revised to be self-contained)